...
import { useStyles } from "@components/posts/components/create-post/styles";
...
} from "@store/ai-services/actions";
import CancelModalComponent from "@components/posts/components/cancel-modal";
import RewriteTextComponent from "@components/posts/components/rewrite-text";
...
export default function CreatePostComponent() {
	...
	const AiTagData = useAppSelector(
		(state) => state.AiServicesReducer.aiTagData,
	);
	const AiModerationData = useAppSelector(
		(state) => state.AiServicesReducer.aiModerationData,
	);
	...
	const processModerationData = useMemo(() => {
		if (!AiModerationData && !AiTagData) {
			return null;
		}

		const nsfwTag = AiModerationData?.replace(/<[^>]*>?/gm, "").trim();
		const genreTag = AiTagData?.replace(/<[^>]*>?/gm, "").trim();
	...
	const renderTags = () => {
		return (
			<TagGroup>
				{AiModerationData === NSFWConstant && (
					<Tag className={styles.nsfwTag}>{AiModerationData}</Tag>
				)}
				{AiTagData && (
					<Tag className={styles.genreTag}>{AiTagData}</Tag>
				)}
			</TagGroup>
		);
	};
