...
public sealed class CommonServicesController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration, ICommonServicesHandler commonServicesHandler) : BaseController(httpContextAccessor, configuration)
{
    /// <summary>
    ...
    public async Task<IActionResult> GetLookupMasterDataAsync()
    {
        ...
        if (result is not null && result.Any()) return HandleSuccessResult(result);
        else return HandleBadRequest(ExceptionConstants.SomethingWentWrongMessageConstant);
    }
}
